...
def sshcon(con):
    ...
    print('Establishing a remote connection now.\n')
...
def reseat(ssh, bladeids,switchids):
    for blade in bladeids:
        ...
        stdin, stdout, stderr = ssh.exec_command('service -T blade['+blade+'] -vr')
       # print(stdout.readlines())
        time.sleep(5)

    for switch in switchids:
        print('Virtually reseating switch['+switch+'] now.\n')
        stdin, stdout, stderr = ssh.exec_command('service -T blade['+switch+'] -vr')
       # print(stdout.readlines())
        time.sleep(600)
    time.sleep()
...
ssh = sshcon(condata)
print('Gathering chassis inventory\n')
