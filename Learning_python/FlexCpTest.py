...
class test_002_Basic_flex_pattern(unittest.TestCase):
    @classmethod
    ...
    '''def test_002(self):#this test creates a basic pattern containing some system information pattern for rack server
        time.sleep(2)
    ...
    def test_001_basic_flex_test(self):#this test creates a basic pattern containing some system information pattern for flex server
        time.sleep(2)
        prov = self.dr.find_element_by_id('provisioning')
        prov.click()
        patterns = self.dr.find_element_by_link_text('Patterns')
        ...
        self.dr.find_element_by_xpath('//span[3][contains(text(),"Save")][not(contains(text(),"Deploy"))]').click()
        time.sleep(2)
        self.assertTrue(self.dr.find_element_by_partial_link_text('Flex Pattern from scratch').is_displayed())
